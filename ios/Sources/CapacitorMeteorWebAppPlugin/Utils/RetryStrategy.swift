...
final class RetryStrategy {
    ...
    func retryIntervalForNumber(ofAttempts numberOfAttempts: UInt) -> TimeInterval {
        ...
        let interval = baseTimeInterval * pow(exponent, Double(numberOfAttempts - numberOfAttemptsAtMinimumTimeInterval))
        let randomizedInterval = interval * (1.0 + randomizationFactor * (Double.random(in: 0...1) * 2.0 - 1.0))
